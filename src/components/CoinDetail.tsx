...
export default function CoinDetail({
  ...
  // ── Order Book renderer (shared between desktop center column and mobile below) ──
  const renderOrderBook = (extraClass?: string) => {
    ...
    return (
      <div className={`${dk ? "border-white/8 bg-[#090909]" : "border-gray-100 bg-gray-50/30"} ${extraClass ?? ""}`}>
  ...
  return (
    <div className="flex-1 flex flex-col overflow-y-auto">
      ...
      {/* ── TOP: 3-column area (shrink-0, doesn't collapse) ──────────────────── */}
      <div className="shrink-0 flex flex-col md:flex-row" style={{ minHeight: 460 }}>

        {/* ── LEFT: Stats + Chart TF selector + Chart canvas ────── */}
        <div className={`flex-1 flex flex-col min-w-0 ${T.chartBg}`}>
          ...
          {/* Chart */}
          <div className="shrink-0 h-[240px] md:h-[400px] relative">
        ...
        {/* ── CENTER: Order Book (desktop only, ~170px) ─────────── */}
        <div className={`hidden md:block w-[170px] shrink-0 border-l overflow-y-auto ${dk ? "border-white/8" : "border-gray-100"}`}>
          {renderOrderBook()}
        </div>
      ...
      <div className={`md:hidden border-t ${dk ? "border-white/8" : "border-gray-100"}`}>
        {renderOrderBook()}
      </div>
