...
async function pollBase() {
  try {
    ...
    const addrToUserId = new Map(
      users.map((u: any) => [u.deposit_address_evm.toLowerCase(), u.id as string])
    );
    ...
    for (const user of users as any[]) {
      try {
        ...
        for (const sigInfo of sigs) {
          ...
          if (seenSolSigs.size > 2000) {
            const iter = seenSolSigs.values();
            seenSolSigs.delete(iter.next().value!);
          }
          ...
          const tx = await solClient.getParsedTransaction(txHash, {
